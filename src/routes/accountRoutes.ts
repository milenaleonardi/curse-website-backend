...
router.post("/create", async (req: Request, res: Response) => {
  ...
  return res.status(response.message === "OK" ? 200 : 400).send(response);
});
...
router.patch("/update/:id", async (req: Request, res: Response) => {
  const response = await controller.updateById(req.body, req.params.id);
  return res.status(response.error ? 400 : 200).send(response);
});
...
router.get("/queryAccount/:name", async (req: Request, res: Response) => {
  const response = await controller.getByName(req.params.name);
  return res.status(response.error ? 400 : 200).send(response);
});
