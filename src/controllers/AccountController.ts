...
export default class AccountController {
  ...
  @Post("/create")
  public async create(
    @Body() body: { name: string; occupation: string; email: string; telephone: string }
  ): Promise<JsonObject> {
  ...
  public async getByName(name: string): Promise<JsonObject> {
    try {
      const data = await AccountModel.find({ name }).select("name -_id");
